...
// const {makeTitleCase,createUsername} = require("../middleware/helperFunctions")
...
//   console.log(user)
//           //check if its a new registration or a login
//         //   if(user.isNew){
//         //     //send welcome mail
//         //     const from = `Hope from ${process.env.SITE_NAME} <no-reply@${process.env.SITE_DOMAIN}>`;
//         //     const subject = `Welcome to ${process.env.SITE_NAME}`;
//         //     const welcomeEmailTemplate = require("../middleware/emails/welcome");
//         //     const messageBody = welcomeEmailTemplate(user._doc);
...
//     //generate jwt
//     const token = jwt.sign({userid: user._doc._id}, process.env.jwtSecret, {
//        expiresIn : 720000
//      })

//      console.log(token)
    
//     //  return res.redirect(`${process.env.BASE_URL}/auth/redirect/${token}`)
//      return res.json({ token, statuscode: "S1" });
//     // return res
//     //   .status(200)
...
//         errors: [
//           {
//             msg: `A ${makeTitleCase(user.authType)} account already exist with this email, Login with ${makeTitleCase(user.authType)} instead `
//           }
//         ]
//       });
//     }
...
//             errors: [
//               {
//                 msg: `A ${makeTitleCase(existingUser.authType)} account already exist with this email, Login with ${makeTitleCase(existingUser.authType)} instead `
//               }
//             ]
//           });
//         }else{
...
//       let user = await UserModel.findOne({ email: req.body.email,  authType : "local"});
//       if (!user) {
//         return res.json({
//           errors: [
//             {
